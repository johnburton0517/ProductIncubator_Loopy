...
function saveToBinary(bitArray, objToPersist, typeIndex, entityBitSize, bitToRefAnyEntity) {
    ...
    for (const i in PERSIST_MODEL[typeIndex]) {
...
function humanReadableJsonPersistProps(objToPersist) {
    ...
    const persist = {};
    // Iterate over the properties defined in the PERSIST_MODEL for the given typeIndex
    for (const i in PERSIST_MODEL[typeIndex]) persist[PERSIST_MODEL[typeIndex][i].name] = PERSIST_MODEL[typeIndex][i].serializeFunc(objToPersist[PERSIST_MODEL[typeIndex][i].name]);
    return persist;
}
...
function legacyJsonPersistProps(objToPersist) {
    ...
    const persistArray = [];
    // Iterate over the properties defined in the PERSIST_MODEL for the given typeIndex
    for (const i in PERSIST_MODEL[typeIndex]) persistArray[i] = PERSIST_MODEL[typeIndex][i].serializeFunc(objToPersist[PERSIST_MODEL[typeIndex][i].name]);
    return persistArray;
}
function legacyJsonRestoreProps(srcArray, targetConfig, typeIndex) {
    ...
    for (const i in PERSIST_MODEL[typeIndex]) {
        ...
        if (typeof srcArray[i] !== 'undefined' && srcArray[i] !== null) { targetConfig[PERSIST_MODEL[typeIndex][i].name] = PERSIST_MODEL[typeIndex][i].deserializeFunc(srcArray[i]); }
    }
    return targetConfig;
}
...
function listStringFields() {
    const stringFields = [];
    for (const typeIndex in EDIT_MODEL) {
        if (EDIT_MODEL.hasOwnProperty(typeIndex)) {
...
function serializeToBinary(embed, bytesSize = true, bytesEntitiesCount = true, bytesAlignSection = true) {
    ...
    for (const entity in entitiesCount) size += entitiesCountVolume[entity] * entitiesSizes[entity] + 8;
    ...
    for (const entity in entitiesCount) bitArray.append(entitiesCount[entity], bitToRefAnyEntity + 1); // exclude loopy globals type
    for (const entity in entitiesSizes) if (entity === 'loopys' || entitiesCount[entity]) bitArray.append(entitiesSizes[entity], 8);
...
function urlToStdB64(urlStr) {
    ...
    return b64.split('_').join('+').split('-').join('/')
        .split('.')
        .join('=');
}
function stdB64ToUrl(b64) {
    b64 = b64.split('+').join('_').split('/').join('-')
        .split('=')
        .join('.')
...
function factoryRatio(bitNumber, ratioRef, signed = false) {
    if (signed) {
        return {
            bit: bitNumber,
            encode: (v) => Math.min(2 ** bitNumber - 1, Math.max(0, Math.round(2 ** (bitNumber - 1) * v / ratioRef) + 2 ** (bitNumber - 1))),
            decode: (v) => Math.round((v - 2 ** (bitNumber - 1)) * ratioRef / 2 ** (bitNumber - 1)),
        };
    }
    return {
        bit: bitNumber,
        encode: (v) => Math.min(2 ** bitNumber - 1, Math.max(0, Math.round(2 ** bitNumber * v / ratioRef))),
        decode: (v) => Math.round(v * ratioRef / 2 ** bitNumber),
    };
}
...
function countEntities(ceil8 = false) {
    ...
    if (ceil8) for (const i in entities)entities[i] = Math.ceil(entities[i] / 8) * 8;
    return entities;
}
...
function entitiesSize(bitToRefAnyEntity, ceil8 = false) {
    const types = get_PERSIST_TYPE_array().map((t) => `${t._CLASS_.toLowerCase()}s`);
    const entities = {};
    for (const i in PERSIST_MODEL) { PERSIST_MODEL.forEach((t, i) => entities[types[i]] = t.reduce((acc, cur) => acc + (typeof cur.bit === 'function' ? cur.bit(bitToRefAnyEntity) : cur.bit) || acc, 0)); }
    if (ceil8) for (const i in entities)entities[i] = Math.ceil(entities[i] / 8) * 8;
    return entities;
}
