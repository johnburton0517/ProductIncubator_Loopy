...
testEqual(
    'set 1 on 1bit at offset 25 (and verify previous and next data are preserved)',
    7,

    async () => initBitArray().set(1, 1, 25).get(3, 24),
);
testEqual('set 42 on 6bit at offset 37', 42, async () => initBitArray().set(42, 6, 37).get(6, 37));
testEqual(
    'set 42 on 7bit at offset 20 (and verify previous and next data are preserved)',
    341,

    async () => initBitArray().set(42, 7, 20).get(9, 19),
);

testEqual(
    'append 1,0,1,42,0 (on 10bits)',
    724,

    async () => initBitArray().append(1).append(0).append(1)
        .append(42)
        .append(0)
        .get(10, 0),
);
...
{
    testEqual(
        'zGet first identical 30bit',
        '[0:24b] 10101000',
        async () => binView((new BitArray(64)).append(42, 30).append(0b1111110001111110, 16).zGet(30, 0, 0).rawData.buffer),
    );
    testEqual(
        'zGet 2nd identical 30bit',
        '[0:24b] 10101000',
        async () => binView((new BitArray(64)).append(42, 30).append(0b1111110001111110, 16).zGet(30, 0, 1).rawData.buffer),
    );

    testEqual(
        'zPush some identical data on 30bit each',
    ...
    testEqual(
        'zPush some similar data on 30bit each',
    ...
    testEqual(
        'zPush some similar data on 100bit each',
    ...
    testEqual(
        'zPush some similar data on 100bit each (and don\'t compress when it\'s not effective)',
